...
const ce = "[-\\+]?\\d+%?", ae = "[-\\+]?\\d*\\.\\d+%?", F = `(?:${ae})|(?:${ce})`, fe = `[\\s|\\(]+(${F})[,|\\s]+(${F})[,|\\s]+(${F})\\s*\\)?`, U = `[\\s|\\(]+(${F})[,|\\s]+(${F})[,|\\s]+(${F})[,|\\s]+(${F})\\s*\\)?`, Ot = new RegExp(`rgb${fe}`), $t = new RegExp(`rgba${U}`), Tt = new RegExp(`hsl${U}`), Ct = new RegExp(`hsla${U}`), Rt = new RegExp(`hsv${U}`), _t = new RegExp(`hsva${U}`), kt = /^#?([0-9a-fA-F]{1})([0-9a-fA-F]{1})([0-9a-fA-F]{1})$/, xt = /^#?([0-9a-fA-F]{1})([0-9a-fA-F]{1})([0-9a-fA-F]{1})([0-9a-fA-F]{1})$/, Dt = /^#?([0-9a-fA-F]{2})([0-9a-fA-F]{2})([0-9a-fA-F]{2})$/, It = /^#?([0-9a-fA-F]{2})([0-9a-fA-F]{2})([0-9a-fA-F]{2})([0-9a-fA-F]{2})$/, W = Object.freeze({
  aliceblue: "f0f8ff",
  antiquewhite: "faebd7",
  aqua: "0ff",
  aquamarine: "7fffd4",
...
function Pn(t) {
  return t = String(t).trim().toLowerCase(), t ? t === "transparent" || W[t] ? !0 : t === "transparent" || ue.has(t) || Ot.test(t) || $t.test(t) || Tt.test(t) || Ct.test(t) || Rt.test(t) || _t.test(t) || kt.test(t) || xt.test(t) || Dt.test(t) || It.test(t) : !1;
}
function le(t) {
...
function O(t) {
  ...
  return typeof t == "string" ? r = le(t) : r = t, r !== null && typeof r == "object" && ("l" in r ? e = Lt(r.h, r.s, r.l) : "v" in r && (e = he(r.h, r.s, r.v)), "a" in r && (n = C(r.a ?? 1), Number.isNaN(n) && (n = 1)), e = r), { ...e, a: n, format: "rgba", toString: M };
}
function z(t, e, n) {
  return {
    h: w(t, 0, 360) / 360,
...
}), Me = 1e3, A = 1e3, k = 60, Se = k * A, fr = k * A, X = 60, zt = X * k, we = zt * A, ur = zt * A, jt = 24, Ee = jt * X, qt = Ee * k, Ne = qt * A, lr = qt * A, ct = 7, Ae = ct * jt, Fe = Ae * X, Kt = Fe * k, Oe = Kt * A, dr = Kt * A, Qt = 3, $e = 4, Te = $e * Qt, Ce = {
  y(t, e) {
...
function $r(t, e, n = 0) {
  return t = Et(t, n), e = Et(e, n), (e.getTime() - t.getTime()) / Oe;
}
function We(t, e) {
  t = h(t), e = h(e);
...
function Qe(t, e = {}) {
  if (t == null || typeof t != "object")
    return t;
  const { cloneObject: n = ot } = e, r = /* @__PURE__ */ Object.create(null), o = [
    {
      parent: r,
      prop: "root",
      data: t
...
const Ze = y && ("ontouchstart" in window || Je() > 0), Lr = Ze ? "pointerdown" : "click";
function Je() {
...
const D = /* @__PURE__ */ new Set(), Jt = /* @__PURE__ */ new WeakMap();
function dn() {
  D.forEach((t) => {
    t(...Jt.get(t));
  }), D.clear();
...
function ro(t, ...e) {
  if (typeof t != "function")
    return t;
  Jt.set(t, e), !D.has(t) && (D.add(t), D.size === 1 && dt(dn));
}
...
function mo(t, e = {}) {
  ...
  for (; b.length; ) {
    ...
    const ht = f(m);
    if (ht && p.push(m), ht || !u) {
      for (let v = 0, te = J.length; v < te; ++v) {
        const gt = J[v];
...
export {
  ...
  Te as YEAR_ON_MONTHS,
  $e as YEAR_ON_QUARTERS,
  at as addDays,
  mr as addHalfDays,
  Le as addHours,
